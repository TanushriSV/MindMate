...
import { Flame, Sparkles, History, Check, ChevronRight, Wind } from 'lucide-react';
    ...
    <div className="space-y-12 pb-12 pt-4 relative">
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <motion.div 
            whileHover={{ y: -5 }}
            onClick={() => triggerTooltip('reflection')}
            className="bg-white rounded-2xl calm-shadow overflow-hidden border border-surface-variant/30 group cursor-pointer relative"
          >
            <div className="h-48 overflow-hidden">
            ...
            <AnimatePresence>
              {activeTooltip === 'reflection' && (
                <motion.div 
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="absolute inset-0 bg-primary/90 backdrop-blur-sm flex flex-col items-center justify-center text-white p-6 text-center"
                >
                  <Sparkles size={32} className="text-tertiary animate-pulse mb-2" />
                  <p className="font-extrabold text-lg">Content coming soon</p>
                  <p className="text-sm text-white/80 mt-1">Our daily writing prompt engine is being integrated for your workspace.</p>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
