...
async function generateFile(filename, category, templatesArray) {
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.readFile('C:\\Users\\Tanushri\\Downloads\\selenium report.xlsx');
  const worksheet = workbook.worksheets[0];
  
  // Clear any existing data below header (row 1)
  ...
  while (count < 300) {
    for (const mod of features) {
      ...
      const newRow = worksheet.addRow([
        ...
        "N/A",                                // Screenshot Path
        "",                                   // Error Message
        new Date().toISOString().split('T')[0] // Execution Date
      ]);
      
      count++;
      if (count % templatesArray.length === 0) {
        variation++;
      }
    }
  }
...
async function run() {
  await generateFile("Selenium_Test_Cases_V2.xlsx", "Selenium", e2eTemplates);
  await generateFile("Load_Test_Cases_V2.xlsx", "Load Testing", loadTemplates);
  await generateFile("Vulnerability_Test_Cases_V2.xlsx", "Vulnerability Validation", secTemplates);
  await generateFile("APM_Test_Cases_V2.xlsx", "APM", apmTemplates);
  console.log("Successfully generated all 4 files matching the template exact format.");
}
