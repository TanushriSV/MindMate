...
export default function SignIn({ onSignIn, onBack }: SignInProps) {
  ...
  const handleGoogleClick = () => {
    ...
    if (!googleId || googleId === 'dummy-google-client-id' || googleId === '') {
      ...
      setTimeout(async () => {
        ...
        try {
          console.log("Calling:", API_BASE_URL + "/api/auth/token");
  ...
  const handleForgotPassword = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(API_BASE_URL + "/api/auth/reset-request", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="w-full max-w-[1140px] grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
        ...
        <div className="md:col-span-6 space-y-8 hidden md:block">
          <div className="space-y-4">
            <h1 className="text-5xl font-extrabold text-primary tracking-tight">{t('auth.heroTitle')}</h1>
            <p className="text-xl text-on-surface-variant max-w-md font-medium leading-relaxed">{t('auth.heroSubtitle')}</p>
          </div>
        ...
        >
          <div className="w-full max-w-[480px] bg-white rounded-3xl calm-shadow p-8 md:p-12 space-y-8 border border-surface-variant/30">
            <div className="text-center md:text-left space-y-2">
              <h2 className="text-3xl font-bold text-on-surface">{isSignUp ? t('auth.createAccount', "Create an account") : t('auth.welcomeBack', "Welcome back")}</h2>
              <p className="text-sm font-medium text-on-surface-variant">{isSignUp ? t('auth.createAccountSubtitle', "Join our mindful community today") : t('auth.signInSubtitle', "Please enter your details to continue")}</p>
            </div>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="flex items-center justify-start px-1">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <span className="text-xs font-bold text-on-surface-variant group-hover:text-on-surface transition-colors">{t('auth.rememberMe')}</span>
                </label>
              </div>
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    {isSignUp ? t('auth.createAccountBtn', "Create account") : t('auth.signInButton', "Sign in")}
                    <ArrowRight size={20} />
                  </>
                )}
              </button>
            ...
            <div className="relative py-4">
              ...
              <div className="relative flex justify-center text-xs font-bold uppercase tracking-widest"><span className="bg-white px-4 text-outline">{t('auth.orContinueWith')}</span></div>
            </div>
            ...
            <p className="text-center text-sm font-medium text-on-surface-variant pt-4">
              {isSignUp ? (
                <>
                  {t('auth.alreadyHaveAccount')} <button type="button" onClick={() => { setIsSignUp(false); setError(null); }} className="text-primary font-bold hover:underline cursor-pointer">{t('auth.signInHere')}</button>
                </>
              ) : (
                <>
                  {t('auth.dontHaveAccount')} <button type="button" onClick={() => { setIsSignUp(true); setError(null); }} className="text-primary font-bold hover:underline cursor-pointer">{t('auth.signUpFree')}</button>
                </>
              )}
            </p>
          </div>
