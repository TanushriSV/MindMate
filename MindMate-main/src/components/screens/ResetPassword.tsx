      ...
      <div className="w-full max-w-[1140px] grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
        ...
        <div className="md:col-span-6 space-y-8 hidden md:block">
          <div className="space-y-4">
            <h1 className="text-5xl font-extrabold text-primary tracking-tight">{t('auth.resetHeroTitle')}</h1>
            <p className="text-xl text-on-surface-variant max-w-md font-medium leading-relaxed">{t('auth.resetHeroSubtitle')}</p>
          </div>
        ...
        >
          <div className="w-full max-w-[480px] bg-white rounded-3xl calm-shadow p-8 md:p-12 space-y-8 border border-surface-variant/30" id="reset-password-card">
            <div className="text-center md:text-left space-y-2">
              <h2 className="text-3xl font-bold text-on-surface">{t('auth.resetPasswordTitle')}</h2>
              {email ? (
                <p className="text-sm font-medium text-on-surface-variant">{t('auth.resettingFor')} <span className="text-primary font-semibold">{email}</span></p>
              ) : (
                <p className="text-sm font-medium text-on-surface-variant">{t('auth.chooseNewPassword')}</p>
              )}
            </div>

            {!token || !email ? (
              <div className="p-4 bg-orange-50 border border-orange-200 rounded-2xl text-orange-700 text-sm font-medium leading-relaxed">
                {t('auth.missingLinkDetails')}
              </div>
            ) : (
