...
function messageSuggestsBreathe(text: string): boolean {
  ...
  return (
    ...
    norm.includes('rest') ||
    norm.includes('reset') ||
    norm.includes('respiración') ||
    norm.includes('respirer')
  );
}
...
export default function Chat() {
  ...
  const confirmDeleteSession = async () => {
    ...
    try {
      ...
      setError(t('chat.deleteSessionError'));
    }
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (err?.code === 'AI_NOT_CONFIGURED') {
        setError(t('chat.aiNotConfigured'));
      } else if (err?.code === 'AI_UNAVAILABLE') {
        setError(t('chat.aiUnavailable'));
      } else {
        setError(t('chat.aiGeneralError'));
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const activeMessages = messages.length > 0 ? messages : [
    {
      id: 'greeting',
      role: 'model' as const,
      text: t('chat.greetingMsg', { name: user?.name ? user.name : 'there' }),
      timestamp: Date.now()
    }
  ];
  ...
  const renderSidebarContent = () => (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between gap-3 mb-6 shrink-0">
        <h3 className="text-lg font-bold text-on-background select-none">{t('chat.myChats')}</h3>
        <button
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1 hide-scrollbar">
        {sessionsLoading ? (
          <div className="flex flex-col items-center justify-center py-12 gap-2 text-on-surface-variant/60">
            ...
            <span className="text-xs font-medium">{t('chat.checkingRooms')}</span>
          </div>
        ) : sessions.length === 0 ? (
          <p className="text-center text-xs font-bold text-on-surface-variant/40 py-10 select-none">{t('chat.noPastChats')}</p>
        ) : (
          sessions.map((sess) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-1.5 opacity-100 transition-opacity ml-2 shrink-0">
                  {isEditing ? (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    <>
                      <button
                        ...
                        className="p-1.5 hover:bg-primary/20 hover:text-primary text-on-surface-variant/70 rounded-md cursor-pointer transition-all"
                        title={t('chat.rename')}
                      >
                        <Edit2 size={13} />
                      </button>
                      <button
                        ...
                        className="p-1.5 bg-error/10 text-error/80 hover:bg-error hover:text-white rounded-md cursor-pointer transition-all"
                        title={t('chat.delete')}
                      >
                        <Trash2 size={13} />
                      </button>
        ...
        <header className="flex items-center justify-between gap-3 pb-3 border-b border-outline-variant/20 shrink-0 select-none">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <h2 className="text-sm font-black text-on-surface-variant uppercase tracking-wider">{t('chat.sanctuaryAiChat')}</h2>
              <p className="text-xs text-primary font-bold truncate max-w-[150px] sm:max-w-xs">
                {sessionsLoading 
                  ...
                  : (() => {
                      ...
                      return activeS_ ? formatSessionTitle(activeS_.title, activeS_.created_at) : t('chat.companion');
                    })()
                }
              </p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-1 py-4 space-y-6 hide-scrollbar pb-28">
          {isLoadingHistory ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 py-20 text-on-surface-variant/50 select-none">
              ...
              <span className="text-xs font-black uppercase tracking-widest text-primary">{t('chat.unfoldingConversation')}</span>
            </div>
          ) : (
            <>
              ...
              {activeMessages.length <= 1 && (
                <div className="text-center py-6 mb-4 shrink-0 select-none animate-fade-in">
                  ...
                  <h4 className="text-xl font-black text-on-background tracking-tight">{t('chat.aiSanctuaryChat')}</h4>
                  <p className="text-xs text-on-surface-variant max-w-sm mx-auto mt-1 leading-relaxed">
                    {t('chat.aiSanctuaryDesc')}
                  </p>
                </div>
              )}
                      ...
                      {m.role === 'model' && messageSuggestsBreathe(m.text) && (
                        <div className="flex justify-start">
                          ...
                          >
                            ...
                            <span>{t('chat.startBreathing')}</span>
                          </button>
                        </div>
                      )}
                    </div>
          ...
          <div className="w-full relative">
            ...
            <div className="bg-surface-container/95 backdrop-blur-xl rounded-[28px] p-2 shadow-[0_8px_30px_rgb(0,0,0,0.08)] flex items-center gap-2 border border-outline-variant/30">
              ...
              <input 
                ...
                placeholder={t('chat.typeThoughts')}
              />
              <button 
                onClick={handleSend}
            ...
            >
              ...
              <div className="flex gap-3 w-full mt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 rounded-full bg-red-500 text-white font-bold text-xs hover:bg-red-600 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer shadow-md"
                >
                  {t('chat.delete')}
                </button>
              </div>
