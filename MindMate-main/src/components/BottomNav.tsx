...
export default memo(function BottomNav({ currentScreen, onNavigate }: BottomNavProps) {
  ...
  const items = [
    { id: Screen.Home, icon: Home, label: t('nav.home', 'Home') },
    { id: Screen.CheckIn, icon: Edit3, label: t('nav.checkin', 'Check-in') },
    { id: Screen.Chat, icon: MessageCircle, label: t('nav.chat', 'AI Chat') },
    { id: Screen.History, icon: BarChart3, label: t('nav.history', 'History') },
    { id: Screen.Profile, icon: User, label: t('nav.profile', 'Profile') },
  ];
