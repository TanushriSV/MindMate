...
export async function generateJournalFeedback(content: string, mood: string): Promise<string> {
  const prompt = `You are MindMate, a highly empathetic and human-like AI companion for a student mental wellness app. The user just wrote a journal entry and indicated they are feeling ${mood}. 

Their journal entry: "${content}"
...
export async function deleteEntry(id: string) {
  const token = getAuthToken();
  const response = await fetch(API_BASE_URL + `/api/entries/${encodeURIComponent(id)}`, {
    method: "DELETE",
    headers: {
      ...(token ? { "Authorization": `Bearer ${token}` } : {})
    }
...
export async function deleteAllEntries() {
  const token = getAuthToken();
  const response = await fetch(API_BASE_URL + "/api/entries/all", {
    method: "DELETE",
    headers: {
      ...(token ? { "Authorization": `Bearer ${token}` } : {})
    }
...
export async function updateChatSessionTitle(id: string, title: string): Promise<{ success: boolean }> {
  const token = getAuthToken();
  const response = await fetch(API_BASE_URL + `/api/chat/sessions/${encodeURIComponent(id)}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      ...(token ? { "Authorization": `Bearer ${token}` } : {})
